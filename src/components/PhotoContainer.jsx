...
const PhotoContainer = () => {
  ...
  const fetchPhotos = async (pageNumber) => {
    setLoading(true); // Set loading state to true when fetch begins
    try {
  ...
  useEffect(() => {
    fetchPhotos(page);
  }, [page]); // Fetch photos whenever the page state changes

  const handleScroll = () => {
    try {
      if (
        !loading && // Only trigger fetch if loading is false
  ...
  return (
    <>
      <section className='text-black max-h-full flex flex-col items-center text-xs md:text-sm font-medium px-5 md:px-24 mt-5'>
        <div className='flex'>
          <p className='md:px-7 px-4 cursor-pointer py-4 md:py-4 rounded-full hover:bg-black hover:text-white'>
            Videos
          </p>
          <p className='md:px-7 px-4 cursor-pointer py-4 md:py-4 rounded-full hover:bg-black hover:text-white'>
            Home
        ...
        <div className='flex justify-between items-center w-full md:mt-5'>
          <div className='py-5 text-xl md:text-2xl font-medium tracking-tight'>
            Free Stock Photos
          </div>
          <button className='md:px-7 px-6 h-12 md:py-7 text-xs md:text-base rounded-md font-medium border-2 flex items-center gap-2'>
        ...
        {error ? (
          ...
          <>
            ...
            {loading && (
              <div className='text-center w-full text-black text-2xl'>
                Loading...
              </div>
            )}
          </>
